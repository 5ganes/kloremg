  ...
  <tr>
    ...
    <td width="<?php echo ADMIN_BODY_WIDTH; ?>" align="center"><span class="hdr">Welcome to Administrative Console of <br><span class="welcome_body_text"><?php echo PAGE_TITLE; ?></span><br>
      ...
      <p style="font-size:20px; font-weight:bold">
      		Total Info Providers: <? $pro=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(id) FROM `usergroups`"); $proGet=mysqli_fetch_array($pro); echo $proGet['count(id)']; ?>
      </p>
      <p style="font-size:20px; font-weight:bold">
      		Total Connected Farmers: <? $far=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(id) FROM `farmer`"); $farGet=mysqli_fetch_array($far); echo $farGet['count(id)']+500; ?>
      </p>
