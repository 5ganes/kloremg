...
if($_GET['type'] == "edit")
{
	...
	$result = mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cropvariety where id='$idd'"));
	extract($result);
	//extract($editRow);
}
if($_POST['type'] == "Save")
...
{
		$delid=$_GET['id'];
		mysqli_query($GLOBALS["___mysqli_ston"], "delete from cropvariety where id='$delid'"); //$groups -> delete($_GET['id']);
		$cropId=$_GET['cropId'];
		?>
							...
							if(isset($_GET['cropId']))
							{
								...
								{?>
									<tr <?php if($counter%2 != 0) echo 'bgcolor="#F7F7F7"'; else echo 'bgcolor="#FFFFFF"'; ?>>
										...
										<td valign="top">
											<? $cropId=$row['cropId'];
											$crop=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select name from crop where 
