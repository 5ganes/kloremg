<?php
...

	{
		...
		{  
			...
			$replycount = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select id,count(id) as count from reply where questionId='$id'"));
...

			{
				...
				$sql=mysqli_query($GLOBALS["___mysqli_ston"], "delete from reply where id='$replyId'");	
