<?php
 	...
 	if(isset($_POST['reset']))
	{
		...
		if($newpass==$confpass)
		{
			...
			$userEmail=mysqli_fetch_assoc(mysqli_query($GLOBALS["___mysqli_ston"], "select email from resetpassword where token='$token'"));
			$mail=$userEmail['email']; //$mail; die();
			mysqli_query($GLOBALS["___mysqli_ston"], "delete from resetpassword where token='$token'");
			
			$password=md5($newpass);
			mysqli_query($GLOBALS["___mysqli_ston"], "update usergroups set password='$password' where email='$mail'");
