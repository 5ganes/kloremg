...
$conn = new Dbconn();
$sql=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select count(id) as maxx from farmer")); $count=$sql['maxx'];
$limit=1000; $i=1;
while($count>0)
{
	$lower=($i-1)*$limit;
	$sql=mysqli_query($GLOBALS["___mysqli_ston"], "select registration_id from farmer limit $lower,$limit"); $i++; $count-=$limit;
	
	$registrationIds=array();
	$j=0;
	while($rId=mysqli_fetch_array($sql))
	{
