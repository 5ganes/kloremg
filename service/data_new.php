...
else if($qtype=="crop")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,name,categoryId,code,contents,publish,image,weight from crop where categoryId='$c' and publish='Yes'";
	}
	else{
...
else if($qtype=="info")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,name,contents,districtIds,cropId,userId,onDate from information where id>'$c' and publish='Yes' order by id ASC";	
	}
	else if(isset($_GET['uid'])){
		$uid=$_GET['uid'];
		$sql="select id,name,contents,districtIds,cropId,userId,onDate from information where userId='$uid' and publish='Yes' order by id ASC";
	}
	else if(isset($_GET['cid'])){
		$cid=$_GET['cid'];
		$sql="select id,name,contents,districtIds,cropId,userId,onDate from information where cropId='$cid' and publish='Yes' order by id ASC";
	}
	else{
		$sql="select id,name,contents,districtIds,cropId,userId,onDate from information where publish='Yes' order by id ASC";	
		//echo $sql;
	}
}
else if($qtype=="cropvariety")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,name,cropId,productionTime,recommendedArea,recommendedDate,productivity,weight from cropvariety where cropId='$c' and publish='Yes' order by weight";
	}
	else
	{
...
else if($qtype=="croptechnology")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,cropId,cropvarietyId,highMountain,mediumMountain,tarai,fertilizer,dyandyan,plantplant,seedRate,weight from croptechnology where cropId='$c' and publish='Yes' order by weight";
	}
	else
	{
		$sql="select id,cropId,cropvarietyId,highMountain,mediumMountain,tarai,fertilizer,dyandyan,plantplant,seedRate,weight from croptechnology where publish='Yes' order by weight";	
	}
}
else if($qtype=="newtechnology")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,cropId,name,description,weight from newtechnology where cropId='$c' and publish='Yes' order by weight";
	}
	else
	{
		$sql="select id,cropId,name,description,weight from newtechnology where publish='Yes' order by weight";	
	}
}
...
else if($qtype=="diary")
{
	if(isset($_GET['c'])){
		...
		$cat=$_GET['cat'];
		$sql="select id,name,categoryId,phone,fax,email,website,weight from diary where categoryId='$cat' and publish='Yes' order by weight";
	}
	else
	{
...
else if($qtype=="infoquesreply"){
	if(isset($_GET['info_id'])){
		$info_id=$_GET['info_id'];
		$sql="select questions.id as question_id,questions.name as name,questions.phone,questions.email,questions.question,questions.deviceId,questions.infoId as info_id,questions.onDate as asked_date, reply.id as reply_id,reply.answer as answer,reply.providerId as info_provider_id,reply.onDate as replied_date from questions
		inner join reply on questions.id=reply.questionId 
		where questions.infoId='$info_id' and questions.publish='Yes' and reply.publish='Yes' 
