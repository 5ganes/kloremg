...
else if($qtype=="croptechnology")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,cropId,cropvarietyId,highMountain,mediumMountain,tarai,fertilizer,dyandyan,plantplant,seedRate,weight from croptechnology where id>'$c' and publish='Yes' order by weight";
	}
	else
	{
...
else if($qtype=="newtechnology")
{
	if(isset($_GET['c'])){
		$c=$_GET['c'];
		$sql="select id,cropId,name,description,weight from newtechnology where id>'$c' and publish='Yes' order by weight";
	}
	else
	{
