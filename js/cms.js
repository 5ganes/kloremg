...
function checkCropUrlName(id, val, div)
{
	$.get('checkcropurlname.php', {id: id, value: val}, function(data){
		$("#" + div).html(data);
	})
}
...
function checkCropUrlName(id, val, div)
{
	$.get('checkdiseaseurlname.php', {id: id, value: val}, function(data){
		$("#" + div).html(data);
	})
}

function checkVideoUrlName(id, val, div)
{
	$.get('checkvideourlname.php', {id: id, value: val}, function(data){
		$("#" + div).html(data);
	})
}
...
function changeCategoryCrop(box)
{
	location.href = "crop.php?categoryId=" + box.value;	
}

function changeDiseaseCrop(box)
{
	location.href = "disease.php?cropId=" + box.value;	
}

function changeVideoCrop(box)
{
	location.href = "video.php?cropId=" + box.value;	
}

function changeImageCrop(box)
{
	//document.getElementById("errCrop").innerHTML="Please Select Crop";
	location.href = "diseaseimage.php?cropId=" + box.value;	
}

function changeImageDisease(box)
{
	var cropId = document.getElementById("cropId");
	var cropId = cropId.options[cropId.selectedIndex].value;
	location.href = "diseaseimage.php?cropId=" + cropId + "&diseaseId=" + box.value;	
}

function addDiseaseImage()
{
	newDiv = document.createElement("div");
	...
	str += "<div style='width:100px; float: left;'>Caption : </div>";
	str += "<div style='float:left;'><input type='text' name='contents[]' class='text' /></div>";
	str += "<hr style='clear: both;'>";
...
function changeCropTechnolgyy(box)
{
	location.href = "croptechnology.php?cropId=" + box.value;	
}

function changeNewTechnolgy(box)
{
	location.href = "newtechnology.php?cropId=" + box.value;	
}